...
class World {
  ...
  attack(player, monId) {
    ...
    if (m.hp <= 0) {
      ...
      return {
        ...
        x: kr.x, y: kr.y                            // kill location, for proximity-credit fan-out (light grouping)
      };
    }
    return {
  ...
  killMonster(m, player, now) {
    const kx = m.x, ky = m.y;                      // where it actually died, before the corpse goes home —
                                                    // callers use this for proximity-credit fan-out (light grouping)
    m.hp = 0;
    ...
    return { loot, xp, level: h.level, levelUp: h.level > before, ripe: m.ripe, x: r1(kx), y: r1(ky) };
  }
