 ...
 * pending STRATUM with no wallet and no fee (src/rewards.js). A player only hits
 * this module once they connect a wallet and ask to:
 *   - claim: turn pending STRATUM into an on-chain payout (server sends `payout`,
 *     the treasury keeps `fee` — that cut never leaves the treasury wallet)
 *   - convert: swap gold <-> pending STRATUM. The fee is STRATUM credited to the
 ...
 *   - Integer STRATUM and gold only. Fee is floor(gross * bps / 10000).
...
(function (root, factory) {
  ...
  /** Whole gold spent to mint one gross STRATUM before the convert fee. */
  var GOLD_PER_STRM = 10;
  /**
   * Smallest gross STRATUM a convert will touch. 2.5% of 40 is 1, so the fee is
   ...
   *   dir 'to-gold'  — `amount` is pending STRATUM to spend (omit to spend all)
   * `balances` is { gold, pending } (wire field names). `opts` is { feeBps, goldPerStrm, minStrm }.
   * On success, to-token spends `spentGold` and mints `payout` pending STRATUM
   * (fee STRATUM goes to the treasury, not the player). to-gold spends `gross`
   * pending STRATUM and grants `goldOut` gold.
   */
